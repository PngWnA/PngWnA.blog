...
for row in rows[1:]:
    if float(row[mag]) <= 7.9:
        ...
        payload.append(dict(reduced))
```
우리가 {% post_link "별헤는밤/0", "전에"%} 이야기했던 데로 **7.9등급 이하의 별**들을 컷해줍시다. 그리고 우리가 필요로 할 것 같은 id(고유 번호), ra(적경), dec(적위), proper(별칭), ci(별 색깔), mag(겉보기 등급)만 지정해서 저장할 변수인 `payload`에 넣어줍시다. 저 과정을 거친다면 `payload`에는
``` python
